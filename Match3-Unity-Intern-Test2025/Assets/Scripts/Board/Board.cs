...
internal void Fill()
    ...
    internal void Fill()
    {
        ...
        // Random cho các vị trí còn lại
        while (itemPool.Count < totalItems)
        {
        ...
        {
            NormalItem.eNormalType temp = itemPool[i];
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                item.SetType(itemPool[poolIndex]);
                poolIndex++;
    ...
    public List<Cell> GetAllFilledCells()
    {
        List<Cell> list = new List<Cell>();
        for (int x = 0; x < boardSizeX; x++)
        {
